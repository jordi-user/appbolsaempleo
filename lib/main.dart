    ...
    try {
      ...
      if (mounted) {
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (_) => PantallaPrincipal(cicloAlumno: ciclo)),
        );
      }
    } catch (e) {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text("Bolsa SGema", style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold)),
              ...
              const SizedBox(height: 30),
              _isLoading ? const CircularProgressIndicator() : SizedBox(width: double.infinity, child: ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.cyanAccent, foregroundColor: Colors.black, padding: const EdgeInsets.all(15)), onPressed: _iniciarSesion, child: const Text("ENTRAR"))),
              TextButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const RegistroScreen())), child: const Text("Crear cuenta", style: TextStyle(color: Colors.cyanAccent))),
            ],
          ),
        ),
      ),
  ...
  Future<void> _registrarse() async {
    ...
    try {
      ...
      if (mounted) {
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (_) => PantallaPrincipal(cicloAlumno: _ciclo)),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Error: $e")));
      ...
      body: Padding(
        ...
        child: Column(children: [
          ...
          DropdownButtonFormField<String>(
            initialValue: _ciclo,
            items: ['DAM', 'DAW', 'ASIR', 'SMR'].map((c) => DropdownMenuItem(value: c, child: Text(c, style: const TextStyle(color: Colors.white)))).toList(),
            dropdownColor: const Color(0xFF333333),
            onChanged: (v) => setState(() => _ciclo = v!),
            decoration: const InputDecoration(labelText: "Ciclo"),
          ),
          const SizedBox(height: 30),
          _isLoading ? const CircularProgressIndicator() : SizedBox(width: double.infinity, child: ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.cyanAccent, foregroundColor: Colors.black, padding: const EdgeInsets.all(15)), onPressed: _registrarse, child: const Text("REGISTRARME"))),
        ]),
      ),
    );
  }
...
class _PantallaPrincipalState extends State<PantallaPrincipal> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(_indiceActual == 0 ? 'Ofertas ${widget.cicloAlumno} 💼' : 'Agenda Eventos 📅'),
        actions: [
          IconButton(
        ...
        return ListView(
          children: snapshot.data!.docs.map((doc) {
            ...
            return Card(
              ...
              child: ListTile(
                title: Text(data['titulo'] ?? '', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.cyanAccent)),
                subtitle: Text(data['empresa'] ?? ''),
                trailing: const Icon(Icons.arrow_forward_ios, size: 16),
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => DetalleOfertaScreen(data: data))),
              ),
            );
          }).toList(),
        );
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                Text("Error al cargar eventos: ${snapshot.error}", textAlign: TextAlign.center),
                const SizedBox(height: 16),
                ElevatedButton(
                  onPressed: () => setState(() {}),
        ...
        if (snapshot.data!.docs.isEmpty) {
          return Center(
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                const Icon(Icons.event_available_outlined, size: 60, color: Colors.grey),
                const SizedBox(height: 16),
                const Text("No hay eventos programados. 😴", style: TextStyle(fontSize: 16, color: Colors.grey)),
                const SizedBox(height: 16),
                ElevatedButton(
                  onPressed: () => setState(() {}),
              ...
              try {
                if (data['fecha'] != null) {
                  ...
                  if (fecha is String) {
                    ...
                    fechaBonita = '${dateTime.day} de ${meses[dateTime.month - 1]} de ${dateTime.year}';
                  }
                }
              } catch (e) {
              ...
              return Card(
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      // Título
                      Text(data['titulo'] ?? 'Evento', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.purpleAccent)),
                      const SizedBox(height: 12),

                      // Ponente
                      Row(children: [const Icon(Icons.person, size: 18, color: Colors.cyanAccent), const SizedBox(width: 8), Flexible(child: Text(data['ponente'] ?? 'Por definir', style: const TextStyle(fontSize: 16, color: Colors.cyanAccent, fontWeight: FontWeight.w500)))]),
                      const SizedBox(height: 10),

                      // Fecha
                      Row(children: [const Icon(Icons.access_time, size: 16, color: Colors.grey), const SizedBox(width: 8), Flexible(child: Text(fechaBonita, style: const TextStyle(color: Colors.grey)))]),
                      const SizedBox(height: 8),

                      // Ubicación
                      Row(children: [const Icon(Icons.location_on, size: 16, color: Colors.grey), const SizedBox(width: 8), Flexible(child: Text(data['lugar'] ?? 'Online', style: const TextStyle(color: Colors.grey)))]),
                      const Divider(color: Colors.grey, height: 20),

                      // Descripción
                      Text(data['descripcion'] ?? 'Sin descripción', style: const TextStyle(fontSize: 14, color: Colors.white70, height: 1.5)),
                    ],
                  ),
                ),
              );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        SizedBox(width: double.infinity, child: ElevatedButton.icon(icon: const Icon(Icons.send), label: const Text("CONTACTAR"), style: ElevatedButton.styleFrom(backgroundColor: Colors.cyanAccent, foregroundColor: Colors.black), onPressed: () => _abrirEnlace(context, data['link'] ?? '')))
      ])),
    );
  }
}